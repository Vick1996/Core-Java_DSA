...
//        System.out.println(highSalDept);
        ...
        List<String> words = List.of("a", "banana", "app", "ap", "appl", "apply", "apple");
        List<String> sortedWords = words.stream().sorted(Comparator.comparing(p -> p)).toList();
        System.out.println(sortedWords);

        //Remove a particular person from person list
        List<Person> personList = new ArrayList<>();
        personList.add(new Person(1, "Babli", "Sharma"));
        personList.add(new Person(2, "Rohit", "Patel"));
        personList.add(new Person(2, "Pratik", "Mont"));

        String combo = "PratikMont";
        personList.removeIf(p -> combo.equals(p.fname+p.lname));
